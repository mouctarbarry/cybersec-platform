...
export function GuideExplorer({ guides }: { guides: GuideMeta[] }) {
  ...
  const results = useMemo(() => {
    let filtered = guides;
    if (active) {
      filtered = filtered.filter((g) => g.category === active);
    }
    return searchGuides(filtered, debouncedQuery);
  }, [guides, active, debouncedQuery]);
    ...
    <>
      ...
      {results.length === 0 ? (
        <p className="py-12 text-center text-muted-foreground">
          Aucun guide trouvé pour &quot;{debouncedQuery}&quot;
        </p>
      ) : (
